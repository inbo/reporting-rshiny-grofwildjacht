...
countGeneticWolves <- function(data, jaartallen = NULL,
  ...
  names(plotData) <- c("year", "group")
	...
	if (summarizeBy == "count") {
      ...
      title <- "Aantal genetisch geïdentificeerde wolven per jaar in Vlaanderen"
...
#' @import shiny
#' @export
countGeneticWolvesServer <- function(id, data, timeRange, preSelected = reactive(NULL)) {
...
#' @inheritParams reportingGrofwild-common-args
#' @export
countGeneticWolvesUI <- function(id, uiText, plotFunction = "countGeneticWolvesUI", context = "description",
  doHide = TRUE) {
