...
if (currentKbo == "admin")
  currentKbo <- na.omit(unique(c(geoData$KboNummer_Toek, schadeData$KboNummer, matchingWbeData$KboNummer_Partij))) else
...
currentWbe <- matchingWbeData$PartijNummer[match(currentKbo, matchingWbeData$KboNummer_Partij)]
currentWbe <- sort(unique(currentWbe[!is.na(currentWbe)]))
...
if (!doDebug | !exists("spatialData"))
  spatialData <- loadShapeData(WBE_NR = currentWbe)
