...
# set year (stops the reset when input$map_region level changes)
observe({
      if(is.null(input$map_year)) {
        results$map_year_value <- defaultYear 
      } else {
        results$map_year_value <- input$map_year
      }
    })
...
output$map_year <- renderUI({
      ...
      div(class = "sliderBlank", 
          sliderInput(inputId = "map_year", label = "Geselecteerd Jaar (kaart)",
              ...
              value = results$map_year_value,
              sep = "", step = 1))
      
    })


# set time (stops the reset when input$map_region level changes)
observe({
      req(input$map_regionLevel)
      if(is.null(input$map_time)) {
        
        minYear <- if (input$map_regionLevel %in% c("faunabeheerzones", "fbz_gemeentes", "utm5"))
              2014 else
              min(results$wild_geoData()$afschotjaar)
        
        results$map_time_value <- c(minYear, defaultYear)
      } else {
        results$map_time_value <- input$map_time
      }
    })
