...
```{r locaties}
biotoopLocaties <- if (req(input$dash_regionLevel) == "flanders") 
  biotoopData[[input$dash_regionLevel]]$regio[1] else 
  input$dash_locaties
``` 
...
htmlwidgets::saveWidget(dash_results$dash_finalMap()$plot, file = tmpFile, selfcontained = FALSE)
...
```{r 'biotoopPlot'}
plotlyReport(barBiotoop(data = subset(biotoopData[[input$dash_regionLevel]], regio %in% biotoopLocaties))$plot)
```
...
myTable <- tableBackground(data = rbind(
    subset(biotoopData[[input$dash_regionLevel]], regio %in% biotoopLocaties), 
...
```{r popKencijfer, eval = ("F18_8" %in% results$dash_showPopulatie()), results = "asis"}
cat("\\newpage")
cat("## Tabel kencijfer\n")

kencijferResult <- dash_results[["dash_F18_8"]]()
resTable <- kencijferResult$table

resTable[,1] <- paste(resTable[,"categorie"], resTable[,"aantal_categorie"], sep = ": ")

cityList <-   na.omit(resTable[,"gemeente"])

colorList <- ifelse((cityList %in% kencijferResult$observed) & (! cityList %in% kencijferResult$shot),
  "#ef8a62",
  ifelse(
    (cityList %in% kencijferResult$observed) & ( cityList %in% kencijferResult$shot), 
    "transparent", "#67a9cf") )
